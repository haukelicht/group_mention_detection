...
# from utils.bayesian_combination.annotator_models.acc import AccuracyAnnotator
# from utils.bayesian_combination.annotator_models.cm import ConfusionMatrixAnnotator
# from utils.bayesian_combination.annotator_models.cv import ConfusionVectorAnnotator
# from utils.bayesian_combination.annotator_models.dynamic_cm import DynamicConfusionMatrixAnnotator
# from utils.bayesian_combination.label_models.GP_label_model import GPCLabelModel
# from utils.bayesian_combination.tagger_wrappers.gpc import GPC
# from utils.bayesian_combination.tagger_wrappers.indfeatures import IndependentFeatures
# from utils.bayesian_combination.label_models.label_model import IndependentLabelModel
from utils.bayesian_combination.label_models.markov_label_model import MarkovLabelModel
# from utils.bayesian_combination.tagger_wrappers.lstm import LSTM
# from utils.bayesian_combination.annotator_models.spam import SpamAnnotator
from utils.bayesian_combination.annotator_models.seq import SequentialAnnotator
...
class BC(object):
	...
	def __init__(
		...
		beta0_factor=1.0,
		nu0=1,
		annotator_model='seq', # only this one available
		taggers=None,
		tagging_scheme='IOB2',
		true_label_model='HMM', # only this one available
		discrete_feature_likelihoods=True,
		model_dir=None,
		embeddings_file=None,
		verbose=False,
		use_lowerbound=True,
		converge_workers_first=False
	):
		"""
		...
		if true_label_model == 'HMM':
			...
			raise NotImplementedError('GPCLabelModel() not available')
		else:
			#self.LM = IndependentLabelModel(np.ones((self.L)) * beta0_factor, self.L, verbose)
			raise NotImplementedError('IndependentLabelModel() not available')
		...
		if annotator_model == 'acc':
			...
			raise NotImplementedError('ConfusionVectorAnnotator() not available')

		elif 'dyn' == annotator_model.split('.')[0]:
			raise NotImplementedError('issue with `annotator_model`')
	...
	def fit_predict(self, C, doc_start=None, features=None, dev_sentences=[], gold_labels=None):
		...
		with Parallel(n_jobs=n_jobs, timeout=99999) as parallel:
	...
	def optimize(self, C, doc_start, features=None, dev_sentences=[],
		...
		def neg_marginal_likelihood(hyperparams, C, doc_start):
			...
			print("Run %i. Lower bound: %.5f, alpha_0 = %s, nu0 scale = %.3f" % (self.opt_runs, lb, str(np.exp(hyperparams[0:-1])), np.exp(hyperparams[-1])))
		...
		print("Optimal hyper-parameters: alpha_0 = %s, nu0 scale = %s" % (np.array2string(np.exp(opt_hyperparams[:-1])), np.array2string(np.exp(opt_hyperparams[-1]))))
	...
	def lowerbound(self):
		...
		if self.verbose:
			print('Computing LB=%.4f: label model and labels=%.4f, annotator model=%.4f, features=%.4f' % (lb, lnpCtB-lnq_Cdata-lnqtB, lnpPi-lnqPi, lnpRho-lnqRho))
