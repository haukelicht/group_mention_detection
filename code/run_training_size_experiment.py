...
args.nrepeats = 5
args.train_chunk_size = 1000

args.metric = 'seqeval-SG_f1'
args.epochs=10
# use best hyperparams identified in experiment model-comparison
...
for i, (tmp, test_idxs) in enumerate(repeats.split(data, groups=sentence_docs)):
    if i == args.nrepeats:
        break
    ...
    # get one group-based train/val split
    folds = GroupKFold(n_splits=round(1/args.dev_size))
    train_idxs, dev_idxs = next(folds.split(tmp, groups=sentence_docs[tmp]))
    
    # shuffle train and val set indexes
    rng.shuffle(train_idxs)
    rng.shuffle(dev_idxs)
    ...
    # split in chunks of train_chunk_size
    chunks = np.arange(len(train_idxs)) // args.train_chunk_size
    
    # iterate over chunk groups (i.e., training set sizes)
    for c in range(max(chunks)):
        run_id = str(f'rep{i+1:02d}-step{c+1:02d}')
        
        # subset train data to idxs in current (and previous chunks)
        trn = train_idxs[chunks <= c]
        
        split_sizes.append((run_id, len(trn), len(dev_idxs), len(test_idxs)))
        
        print('-'*53)
        print(f'{run_id}: # train: {len(trn)}; # dev: {len(dev_idxs)}; # test: {len(test_idxs)}')
